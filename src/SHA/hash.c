...
char* printHash(uint8_t* hash) {
	int i;
	//char hash_result[20];
	for (i=0; i<20; i++) {
		//hash_result[i]=hash[i];
		sprintf(hash_result+2*i,"%02x",hash[i]);
	}
	hash_result[40]='\0';
	return hash_result;
}
...
char *hash_this(char *client, char *site, char *ucount, char *mcount, char *ccount, char *wcount, char *t_from, char *t_till, int cookie) {
  ...
  char *prehash = _fudge(client, site, ucount, mcount, ccount, wcount, t_from, t_till, cookie);
  sha1nfo s;
  sha1_init(&s);
  sha1_write(&s,prehash,strlen(prehash));
