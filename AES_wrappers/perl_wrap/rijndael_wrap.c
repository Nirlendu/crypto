...
#define SWIG_init    boot_rijndael

#define SWIG_name   "rijndaelc::boot_rijndael"
#define SWIG_prefix "rijndaelc::"
...
#ifdef PERL_OBJECT
#define MAGIC_CLASS _wrap_rijndael_var::
class _wrap_rijndael_var : public CPerlObj {
public:
#else
#define MAGIC_CLASS
#endif
...
XS(_wrap_enc) {
  {
    ...
    if ((items < 2) || (items > 2)) {
      SWIG_croak("Usage: enc(password,filename);");
    }
...
XS(_wrap_dec) {
  {
    ...
    if ((items < 2) || (items > 2)) {
      SWIG_croak("Usage: dec(password,filename);");
    }
    res1 = SWIG_AsCharPtrAndSize(ST(0), &buf1, NULL, &alloc1);
    if (!SWIG_IsOK(res1)) {
...
XS(_wrap_enc_dec) {
  {
    ...
    if ((items < 2) || (items > 2)) {
      SWIG_croak("Usage: enc_dec(password,filename);");
    }
    res1 = SWIG_AsCharPtrAndSize(ST(0), &buf1, NULL, &alloc1);
    if (!SWIG_IsOK(res1)) {
...
{"rijndaelc::enc", _wrap_enc},
{"rijndaelc::dec", _wrap_dec},
{"rijndaelc::enc_dec", _wrap_enc_dec},
{0,0}
};
/* -----------------------------------------------------------------------------
 * Type initialization:
